...
- **Provider abstraction.** `providers/base.py` defines a 5-method `Provider` protocol + a `Capabilities` dataclass. There is no `if provider == "status200"` anywhere outside `providers/` — adding a provider is a new file plus a registry entry (mirrors `batch.OPERATIONS`). Two adapters ship: **Status 200** (`providers/status200.py`) and **Zernio** (`providers/zernio.py`). They differ in ways that change orchestration rather than wire format, and every difference is *declared* in `Capabilities`: Zernio has `supports_status_lookup=True` (a submitted post can be asked about), `supports_account_listing=True` (the admin UI can show which accounts a key reaches), and `supports_remote_schedule=True` + `supports_cancel_scheduled=True` (it genuinely holds a schedule) — Status 200 has none of these. `providers/fake.py` mirrors Status 200's capabilities exactly and backs `PUBLISHING_DRY_RUN=1`, so the full pipeline runs with no credential and no real post — note its `fetch_status` returns `None` on purpose, which makes the poller inherently a no-op in dry run.
...
- **One group, several provider accounts — `credential_slot`.** Zernio's free tier connects 2 social accounts per Zernio account and a full fan-out needs 3, so a group holds more than one key: nullable `credential_slot` on both `publish_credentials` and `publish_destinations`, resolved **slot-first-then-group-default** by `service.credential_for_destination`. Splitting across two *groups* would work at the provider and be wrong here — a group is the unit an operator schedules, assigns clips to and reads capacity for. Resolution is deliberately **one-directional**: a destination naming a slot with no credential reports `no_credential` rather than falling back, because falling back would post to whichever account happened to be the default and return a plausible success. `GET /api/publishing/admin/groups/{id}/accounts` lists what each slot's key can see, so slots are mapped from real account refs instead of guesses.
...
- **A pollable provider is asked before it is condemned.** `dispatcher.poll_submitted_attempts` runs one pass per reconciliation, *before* `sweep_stale_submitted` and in its own transaction: a post that can be asked about must be asked before the timeout ages it into terminal `unknown`, and provider I/O must not hold the sweeps' transaction open. It changes nothing for Status 200. An answer routes through the same service calls a submit or webhook would — `fetch_status` returning a `SubmitResult` with `status="succeeded"` is applied webhook-style (**not** `record_success`, which would stamp `submitted_at = now` and destroy the one timestamp that reconstructs a lost callback), `None` means *no information* (unreachable, or a 404 a deleted post and a never-valid ref share) and is not failure, and a raised `ProviderError` goes to `record_failure` **unless `is_ambiguous`** — recording an ambiguous verdict as failure could retry a live post. Safe by construction: a Zernio post-level verdict can reach `E_ACCOUNT_AUTH` (one destination) but never `E_AUTH` (`CREDENTIAL_FATAL`), so a poll cannot disable a group's key. The rate limit is the **persisted** `publish_attempts.last_polled_at` (not in-memory state — two processes share the queue and either may restart), stamped *before* the call so an endpoint that 500s every lookup is still limited; `state.poll_is_due` is the pure CI-testable statement of the three gates and the SQL `WHERE` is an index-friendly pre-filter it re-checks, so drift can only skip a poll, never add one.
- **The submit timeout runs from the last moment the provider was expected to still be holding the post.** Not from the submit: `state.confirmation_is_overdue` waits out the later of `attempt.deferred_until` (a window the provider asked for) and `request.scheduled_for` (a schedule the provider agreed to keep). The second is not redundant with the first — remote-schedule hand-over submits *immediately* with `scheduledFor` and `promote_remote_schedules` **clears** `deferred_until`, because the local clock is no longer in charge. Reading only `deferred_until` condemned every remote-scheduled post: a rhythm slot hours out against a 30-minute timeout hit terminal `unknown` long before it was due to go live, so the sweeper destroyed exactly the capability hand-over exists to provide. Same pure-rule-plus-pre-filter shape as the poller: drift can only spare a post, never condemn one.
...
- **Scheduling: Status 200 can't hold a slot, Zernio can.** Status 200 accepts `post.scheduledFor` and posts immediately anyway (measured over three real posts; TikTok's and Instagram's APIs expose no scheduled publish either), so it declares `supports_remote_schedule=False` and its slots are held by a local clock — which means **something must be running when a Status 200 slot arrives.** Zernio declares `supports_remote_schedule=True` **and** `supports_cancel_scheduled=True` (verified against its OpenAPI doc and live probes): `scheduledFor`+`timezone` returns `status: "scheduled"` and `DELETE /v1/posts/{id}` cancels it. The orchestrator refuses to hand the clock over without both — an uncancellable scheduled post is uncontrollable — so for **Zernio destinations nothing of ours needs to be awake when the slot arrives**, which is exactly the constraint the `deploy/publisher` always-on runbook exists to work around. The `remote_schedule_*` fallbacks stay armed either way: the day a provider starts accepting-and-ignoring the field, hand-over disables itself after one post, not a day's worth. Two extra entrypoints move that clock off a laptop that sleeps — `publishing/runner.py` (always-on process) and `publishing/tick.py` (one pass then exit, for scheduled/serverless hosts). Both share one Postgres with the app and neither imports `app` or `main`; `tests/test_publishing_runner.py` and `tests/test_publishing_tick.py` enforce that in a subprocess, because a stray import would make the slim image unbuildable and nothing else would notice. It works because a submit reads no local bytes: the provider fetches the clip from the staging bucket by presigned URL. Deployment runbook: [`deploy/publisher/README.md`](deploy/publisher/README.md) (a free Hugging Face Space runs `runner.py`, a Cloudflare Worker cron keeps it from idling, Supabase holds the shared queue — all card-free; a box you control is Appendix A). Modal was the original plan and its working deployment is kept in `deploy/publisher/modal_app.py`, demoted to Appendix B because signup requires a credit card.
...
- **Migrations.** The repo boots via `create_all`; every revision is written defensively (checks the live catalogue before creating anything) so it's safe whether or not the tables already exist. Chain: `20260809_publishing_baseline` → `20260823_credential_slots` (the `credential_slot` columns) → `20260824_status_poll` (`publish_attempts.last_polled_at`).
- **Tests** (`tests/test_publishing_*.py`) run in CI with no Postgres, no credentials, no network — pure-logic suites plus provider contract suites against `httpx.MockTransport` (`test_publishing_provider.py`, `test_publishing_zernio.py`), a stub-session suite for the poller (`test_publishing_poll.py`) and one for the stale sweeper (`test_publishing_sweeper.py`, which also compiles the sweeper's SQL against the Postgres dialect). `test_publishing_e2e.py` self-skips without Postgres; that is expected locally, not a failure.
