...
- **Tests** (`tests/test_publishing_*.py`) run in CI with no Postgres, no credentials, no network — pure-logic suites plus provider contract suites against `httpx.MockTransport` (`test_publishing_provider.py`, `test_publishing_zernio.py`) and a stub-session suite for the poller (`test_publishing_poll.py`). `test_publishing_e2e.py` self-skips without Postgres; that is expected locally, not a failure.
